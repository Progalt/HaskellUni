...
describePlaceTemperatures places = [ ( place, describeTemperature temp ) | ( place, temp ) <- places]
-- *** Uncomment the line above and add your code here ***
...
-- hotter minTemp places = [ (place) | (place, temp) <- places, temp > minTemp ]
hotter _ [] = []
hotter minTemp ((place, temp):list) 
  | temp > minTemp = [place] ++ hotter minTemp list
  | otherwise = hotter minTemp list
...
insert value (Node nodeValue next)
  | value > nodeValue = (Node nodeValue (insert value next))
  | otherwise = (Node value (Node nodeValue next))
